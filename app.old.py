...
def webhook():
    ...
    if passphrase != SECRET_KEY:
        ...
        return {"code": "fail", "message": "denied : nice try."}
    ...
    balance_key = "balance"
    balance = float(
        client.futures_account_balance()[balance_index][balance_key]
    )
    ...
    if action == "CloseShort":
        posiAmt = float(
            client.futures_position_information(symbol=symbol)[ids][
                "positionAmt"
            ]
        )
        print(
            "Short Position amount:",
            float(
                client.futures_position_information(symbol=symbol)[ids][
                    "positionAmt"
                ]
        ...
        if posiAmt < 0.0:
            ...
            close_SELL = client.futures_create_order(
                symbol=symbol,
                positionSide=Sside,
                side="BUY",
                type="MARKET",
    ...
    if action == "CloseLong":
        posiAmt = float(
            client.futures_position_information(symbol=symbol)[idl][
                "positionAmt"
            ]
        )
        print(
            "Long Position amount:",
            float(
                client.futures_position_information(symbol=symbol)[idl][
                    "positionAmt"
                ]
        ...
        if posiAmt > 0.0:
            ...
            close_BUY = client.futures_create_order(
                symbol=symbol,
                positionSide=Lside,
                side="SELL",
                type="MARKET",
    ...
    if action == "OpenShort":
        ...
        if amount[0] == "$":
            ...
            print(
                "SELL/SHORT by USDT amount=", usdt, ": COIN", round(usdt, 30)
            )
        print(
            "Confirm:",
            symbol,
    ...
    if action == "test":
        ...
        if currentmode["dualSidePosition"] != True:
            amount = float(
                client.futures_position_information(symbol=symbol)[0][
                    "positionAmt"
                ]
            )
            if amount > 0:
        ...
        # Hedge
            ...
            amoutL = abs(
                float(
                    client.futures_position_information(symbol=symbol)[1][
                        "positionAmt"
                    ]
                )
            )
            ...
            if amoutS > 0:
                ...
                print("---------------------------")
        print(
            "If position amount is = your real position in binance you are good to GO!"
        )
    ...
    return {"code": "success", "message": "Oki"}
